# Auto-generated generator (corrected). Writes lunarcomms/regolith/dielectric.py.
# Loss tangent uses VALIDATED Siegler form 10**(a'+f**b'), array-safe, not fabricated.
content = r'''"""Dielectric properties of lunar regolith (ASCII-only).
...
'''
import os
os.makedirs(os.path.dirname("lunarcomms/regolith/dielectric.py"), exist_ok=True)
with open("lunarcomms/regolith/dielectric.py", "w") as f:
    f.write(content)
print("Wrote lunarcomms/regolith/dielectric.py (" + str(content.count("def ")) + " functions)")