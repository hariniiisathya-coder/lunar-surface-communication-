...
if exist('nrTDLChannel', 'class') == 8
    ...
    x = (randn(SR/1000*40, 1) + 1j*randn(SR/1000*40, 1)) / sqrt(2);  % 40 ms probe

    for i = picks
        keep = S.AveragePathGains(i, :) > -250;   % strip padding rows
        % NormalizePathGains=false is REQUIRED: nrTDLChannel normalises the
        % total path power to 0 dB by default, which would erase the absolute
        % two-ray gain (a single tap would always read ~0 dB). We want the
        % channel to apply the AveragePathGains we computed.
        tdl = nrTDLChannel( ...
            'DelayProfile',        'Custom', ...
            ...
            'KFactorFirstTap',     40, ...        % ray-based taps ~deterministic
