...
Tests the prediction that coverage should collapse UHF -> S -> Ka on real
terrain, since the Deygout diffraction parameter nu scales as sqrt(f):
shadows deepen sharply with frequency.

Produces:
  - a printed table (Friis %, terrain-aware %, penalty %, UHF-clamp fraction)
  - threeband_coverage.png: coverage (and penalty) vs. frequency
...
def main():
    ...
    # ---- figure: coverage and penalty vs frequency ---------------------
    order = ["UHF", "S", "Ka"]
